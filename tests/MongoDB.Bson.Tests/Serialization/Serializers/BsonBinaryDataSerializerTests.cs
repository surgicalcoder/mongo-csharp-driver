...
#pragma warning disable 618
            var subject = new BsonBinaryDataSerializer();
            var allGuidRepresentations = new[]
            {
            ...
            foreach (var mode in TemporaryGuidRepresentationModes.All)
            {
                foreach (var writerGuidRepresentation in allGuidRepresentations)
                {
                    foreach (var valueGuidRepresentation in allGuidRepresentations)
                    {
                        using (mode.Set())
                        {
                            var mockWriter = new Mock<IBsonWriter>();
